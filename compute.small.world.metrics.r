...
small.world.properties <- function (in.g, N, compress.rds=TRUE, ...) {
    ## this is heavily based on analysis_random_graphs from brainGraph
    ## by Chris Watson
    ## https://github.com/cwatson/brainGraph/blob/master/R/analysis_random_graphs.R
    ...
    if (is.null(names(in.g))) {
        stop("*** The name of the list elements in in.g cannot be NULL\n")
    }

    phi.norm <- vector('list', length=length(in.g))
    start=Sys.time()
    for (i in seq_along(in.g)) {
        subject.start.time=Sys.time()
        ## set the subject name
        subject.name=names(in.g)[i]
        phi.norm[[i]] <- vector('list', length=length(in.g[[i]]))

        cat(sprintf("*** Starting simulation of random graphs for subject %s (%03d of %03d) at %s\n",
                    subject.name, i, length(in.g), format(Sys.time(), '%H:%M:%S')))
        progbar <- txtProgressBar(min=0, max=length(in.g[[i]]), style=3)

        for (j in seq_along(in.g[[i]])) {
            
            rand <- sim.rand.graph.par(in.g[[i]][[j]], N, use.parallel.set.attributes=FALSE, ...)
            ## print(N)
            ## print(length(rand))
            ## print(vapply(rand, is_igraph, logical(1)))
            ## print(sapply(rand, class))
            saveRDS(rand, compress=compress.rds,
                    file=file.path(group.results.dir, '/',
                        sprintf('rand_%s_thr_%02i%s', subject.name, j, '.rds')))
            phi.norm[[i]][[j]] <- rich.club.norm(in.g[[i]][[j]], rand=rand)
            rm(rand)
            ## gc()
    ...
    cat("*** Simulation of", N, "random graphs for", length(in.g[[1]]),
        "densities for each of", length(in.g), "subjects took",
        format(as.chron(end) - as.chron(start)), "\n")  
    ## end of random graph simulations

    ## Get all small-worldness and random measures, all thresholds
    ## ###########################################################
    ## groups <- covars[, levels(Group)]
    subjects = names(in.g)
    fnames <- rand.dt <- small.dt <- vector('list', length=length(in.g))
    
    densities <- names(in.g[[1]])
    for (i in seq_along(in.g)) {
        subject.name=names(in.g)[i]
        subjects.group=as.character(characteristics.df[characteristics.df$Study.ID == subject.name, "Group"])
        cat("*** Computing small world network metrics for subject", subject.name, "\n")
        
        fnames <- list.files(group.results.dir, sprintf('rand_%s_thr.*', subject.name), full.names=T)
        ## print(fnames)
        rand_all <- lapply(fnames, readRDS)
        ## print(length(rand_all))
        saveRDS(rand_all, compress=compress.rds,
                file=file.path(group.results.dir, paste('rand', subject.name, 'all.rds', sep="_")))
        small.dt[[i]] <- small.world(in.g[[i]], rand_all)
        small.dt[[i]][, Study.ID := rep(subject.name,   length(densities))]
        small.dt[[i]][, Group    := rep(subjects.group, length(densities))]
        ## print(dim(small.dt[[i]]))
        ## print(small.dt[[i]])
        
        rand.dt[[i]] <- create.rand.dt(rand_all, subject.name, subjects.group, V1=densities, N)
        ## print(dim(rand.dt[[i]]))
        ## print(rand.dt[[i]])
    ...
    ## for each subject
    ##
    ## rand.dt is a data table containing the Cp, and Lp for each fo
    ## the random networks used to create the small small world
    ## properties
    
    rand.dt <- rbindlist(rand.dt)
    small.dt <- rbindlist(small.dt)
    ## small.dt[, Study.ID:=rep(names(in.g), times=length(in.g))]
